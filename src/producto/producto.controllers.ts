...
export const mostrarProductos = async (req: Request, res: Response) => {
    ...
    const [count, data] = await Promise.all([Producto.estimatedDocumentCount(), Producto.find({}, {}, {...paginationParams})]);
...
export const actualizarProducto = async(req: Request, res: Response) => {
    const { id } = req.params;
    try{
        const productoActualizado = await Producto.findOneAndUpdate({_id: id}, { $set: req.body }, { new: true } );
        return res.status(201).json({
            success: true,
