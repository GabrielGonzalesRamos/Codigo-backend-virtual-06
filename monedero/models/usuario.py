...
class UsuarioModel(base_de_datos.Model):
    ...
    usuarioId = Column(name='id', type_=types.Integer, primary_key=True,unique=True, autoincrement=True, nullable=False)
    ...
    usuarioCorreo = Column(name='correo', type_=types.String(25), nullable=False, unique=True)
    ...
    def __init__(self, nombre, apellido, correo, password):
        ...
        # primero agarro el password y lo convierto en byte mediante 
        # el formato de escritura utf-8 
        passwordBytes = bytes(password, "utf-8")
        # El método hashpw agarra la contraseña y un salt generando aleatoriamente
        # para fusionarlo con la contraseña hasheada (en forma de un hash)
        passwordHash = bcrypt.hashpw(passwordBytes, bcrypt.gensalt())
        # El metodo decode convierte el tipo de dato byte a str
